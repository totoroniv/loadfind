...
class KakaoAddressSearchServiceRetryTest extends AbstractIntegrationContainerBaseTest {
    ...
    def "requestAddressSearch retry success" () {
        ...
        def kakaApiResult = kakaoAddressSearchService.requestAddressSearch(inputAddress)


        then:
        2 * kakaoUriBuilderService.buildUriByAddressSearch(inputAddress)  >> uri
        kakaApiResult.getDocumentList().size() == 1
        kakaApiResult.getMetaDto().totalCount == 1
        kakaApiResult.getDocumentList().get(0).getAddressName() == inputAddress
